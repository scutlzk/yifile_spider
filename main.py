...
def some_try(tes):
    lis = tes.image_to_string(Image.open(os.path.join(
        cur_path, 'cp1.PNG')), lang='eng', builder=pyocr.builders.LineBoxBuilder())
...
class captcha(object):
    ...
    def optimize(self, outname=''):
        if self.path.endswith('png') and self.__sp_png():
            ...
            self.imf = self.imf.filter(ImageFilter.MedianFilter(1))
            # enhance()的参数factor决定着图像的对比度情况。从0.1到0.5，再到0.8，2.0，图像的对比度依次增大.0.0为纯灰色图像;1.0为保持原始
            self.imf = ImageEnhance.Contrast(self.imf).enhance(1.5)
...
class yifile(sv_captcha):
    ...
    def __prepare(self):
        ...
        self.browser.execute_script(
            'document.getElementById("bootyz1").style.display="block"')
        self.browser.execute_script(
            'document.getElementById("bootyz2").style.display="block"')
        self.browser.execute_script(
            'document.getElementById("bootyz3").style.display="block"')
        return True
